...
| Path | Purpose |
| --- | --- |
| `/.well-known/apple-app-site-association` | Passkey (PRF) domain association — the file that activates ADR 0002's end-state encryption. JSON with `webcredentials.apps = ["<TEAMID>.com.bolandcompany.satchel.MessagesExtension"]`. Must be served as `application/json`, no redirect. |
